...
; BPLCON1, 1 WAIT,Largh_plasm CMOVEs per il plasma
...
;	Puntiamo i bitplanes nelle copperlist
	...
	move	d0,14(a1)	; scrive in copperlist 1 
	move	d0,14(a2)	; scrive in copperlist 2
	swap	d0
	move	d0,10(a1)	; scrive in copperlist 1
	move	d0,10(a2)	; scrive in copperlist 2
...
; D0=0
	...
	move	#-40,bpl1mod(a5)	; BPL1MOD = -40 ripete sempre la stessa
					; riga
	move	#0,bpl2mod(a5)		; BPL2MOD = 0 normale

	move	#$2200,bplcon0(a5)	; BPLCON0 - 1 bitplane attivo
...
DoOriz:
	...
	lea	4*4+3(a1),a1		; indirizzo secondo byte della seconda
...
InitPlasma:
	...
	move.l	#$3e43FFFE,d0	; carica la prima istruzione wait in D0.
				...
				; orizzontale $42
	move	#color00+2*2,d1	; mette in D1 la prima meta` di un istruzione 
				; CMOVE in COLOR02 (=$dff184)
	move	#color00+3*2,d4	; mette in D4 la prima meta` di un istruzione 
				; CMOVE in COLOR03 (=$dff186)
	move	#bplcon1,d5	; mette in D5 la prima meta` di un istruzione 
...
DoPlasma:
	...
	move.l	draw_clist(pc),a1	; indirizzo copperlist dove scrivere
	lea	4*(4+2)+2(a1),a1	; indirizzo seconda word della prima
					; colonna del plasma
...
; legge e modifica indice componente B
	...
	moveq	#Largh_plasm-1,d2	; loop ripetuto per tutta la larghezza
...
Pic:	incbin	Plasma/plasm_msk.raw
...
(risparmiamo le ultime 2 CMOVEs di ogni riga; confrontate i diversi
