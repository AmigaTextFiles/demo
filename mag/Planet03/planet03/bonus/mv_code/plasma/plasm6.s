...
NuovaRigaB	equ	4		; come "NuovaRigaR" ma per componente B
...
WaitBlit_init:
	...
	move	#$8000,bltcon1(a5)	; BLTCON1 - shift 8 pixel canale B
					; (usato per il plasma)
...
; D0=0
	...
	move	#$30b8,diwstrt(a5)	; DiwStrt - usiamo una finestra piu`
					; piccola dello schermo
	move	#$ee90,diwstop(a5)	; DiwStop
	move	#$0038,ddfstrt(a5)	; DDFStrt - vengono fetchati 40 bytes
	move	#$00d0,ddfstop(a5)	; DDFStop
	move	#$0080,bplcon1(a5)	; BPLCON1 - i planes pari sono shiftati
					; di 8 pixel a destra
	move	d0,bplcon2(a5)		; BPLCON1/2
	move	#4,bpl1mod(a5)		; BPL1MOD = 4 - fetcha 40 bytes su 44
	move	#4,bpl2mod(a5)		; BPL2MOD = 4 - fetcha 40 bytes su 44

	move	#$4200,bplcon0(a5)	; BPLCON0 - 1 bitplane attivo
...
DoOriz:
	...
	move	#1*64+19,d2		; dimensione blittata:
					; larghezza 40 bytes
					; altezza 1 linea
...
OrizLoop:
	...
	and	#$000f,d0		; lascia solo i 4 bit bassi
	ror	#4,d0			; li sposta nelle prime posizioni
	or	#$09f0,d0		; valore da scrivere in BLTCON0

	asr	#4,d1			
	add	d1,d1			; calcola numero di bytes
...
.WaitBlit
	...
	move	d0,bltcon0(a5)		; BLTCON0 - copia da A a D con shift
	move.l	a3,bltapt(a5)		; BLTAPT - indirizzo sorgente
	move.l	a2,bltdpt(a5)		; BLTDPT - indirizzo destinazione
	move	d2,bltsize(a5)		; BLTSIZE
...
; copperlist 1
	move	#color00+10*2,(a0)+	; colore 10
	addq.l	#2,a0			; spazio per la seconda parte
					; della CMOVE

	move	#color00+13*2,(a0)+	; colore 13
	addq.l	#2,a0
	
	move	#color00+6*2,(a0)+	; colore 6
	addq.l	#2,a0

	move	#color00+11*2,(a0)+	; colore 11
	addq.l	#2,a0

	move	#color00+5*2,(a0)+	; colore 5
	addq.l	#2,a0

	move	#color00+2*2,(a0)+	; colore 2
	addq.l	#2,a0

	move	#color00+9*2,(a0)+	; colore 9
	addq.l	#2,a0

	move	#color00+4*2,(a0)+	; colore 4
	addq.l	#2,a0

; copperlist 2
	move	#color00+10*2,(a1)+	; colore 10
	addq.l	#2,a1			; spazio per la seconda parte
					; della CMOVE

	move	#color00+13*2,(a1)+	; colore 13
	addq.l	#2,a1
	
	move	#color00+6*2,(a1)+	; colore 6
	addq.l	#2,a1

	move	#color00+11*2,(a1)+	; colore 11
	addq.l	#2,a1

	move	#color00+5*2,(a1)+	; colore 5
	addq.l	#2,a1

	move	#color00+2*2,(a1)+	; colore 2
	addq.l	#2,a1

	move	#color00+9*2,(a1)+	; colore 9
	addq.l	#2,a1

	move	#color00+4*2,(a1)+	; colore 4
	addq.l	#2,a1
...
; inizializza i registri blitter per il plasma
.WaitBlit
	btst	#6,dmaconr(a5)		; aspetta il blitter
	bne.s	.WaitBlit

	move	#$4FFE,bltcon0(a5)	; BLTCON0 - D=A+B+C, shift A = 4 pixel
...
COPPERLIST1:
	...
	cmove	0,bplpt+4		; secondo bitplane
	cmove	0,bplpt+6		; word bassa bpl2pt
	cmove	0,bplpt+8		; bpl3pt
	cmove	0,bplpt+10
	cmove	0,bplpt+12		; bpl4pt
	cmove	0,bplpt+14
...
COPPERLIST2:
	cmove	0,bplpt			; bpl1pt
	cmove	0,bplpt+2
	cmove	0,bplpt+4		; bpl2pt
	cmove	0,bplpt+6
	cmove	0,bplpt+8		; bpl3pt
	cmove	0,bplpt+10
	cmove	0,bplpt+12		; bpl4pt
