;*---------------------------------------------------------------------------
;  :Program.    Quest
;  :Contents.   "One Stone" Intro
;  :Author.     Stefano "Max Headroom/TEX" Pucino
;  :Requires.   Quest-Intro (ByteKiller v1.3 packed, 138.196 bytes)
;  :Copyright.  Open Source (like GNU)
;  :Language.   68000 Assembler
;  :Translator. ASM-One v1.46 / Barfly v2.00
;  :To Do.
;               1. Find out why the Quitkey doesn't work
;               2. And why HRTMon doesn't work with WHD, too.
;
;  :History.
...
;
;               27.09.2001 - Version 1.0
;                NEW: Fixed SoundTracker-bug
;                     Commented the whole source
...
;======================================================================
  ...
  IFD BARFLY
   OUTPUT  "Quest-OneStone.slave"           ; Name of the slave
...
_name
    dc.b    "One Stone",0                ; Full name of the intro
_copy
    dc.b    "Quest",0                    ; Copyright information
_info
...
; First we put a "breakpoint" after the decrunch-routine.
; It will jump to the FIX-routine instead. The FIX-routine will start the
; demo afterwards.

    move.w  #$4ef9,$aa(a1)                  ; Write JSR at INTROSTART+$aa
    pea     (_fix,pc)                       ; Throw fix-routine to a7
    move.l  (a7)+,$ac(a1)                   ; And put it after the JSR command
...
_fix
    move.w  #$4eb9,$4e27e                   ; Poke JSR to destination
    pea     (_dbra,pc)                      ; Put SoundTracker-fix on Stack
    move.l  (a7)+,$4e280                    ; Write Stack to destination

    move.w  #$4e71,$4e284                   ; Add 2 NOP-words after the call
    move.w  #$4e71,$4e286                   ; to overwrite old command

    jmp     $20000                           ; Start the intro now !
...
_exe        dc.b    "Quest-OneStone",0      ; Name of the Intro
    even
