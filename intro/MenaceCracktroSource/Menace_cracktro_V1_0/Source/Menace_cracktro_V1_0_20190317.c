/*
  Menace_cracktro.c V1.0 20190317
  ...
  Version history:
  - V1.0 20190317, first public release
...
#define PADDINGH 10 /* minimum number of rows necessary (above the scroll buffer) to clear the background */
#define PADDINGAREA ( PADDINGH * 1 * 42 ) /* size in bytes */

/* define the blank padding space above the expanded scroll buffer, visible on screen */
#define PADDINGBPLAREA ( PADDINGH * 1 * 40 ) /* size in bytes */



const BYTE ID[] = "$VER: Menace_cracktro V1.0 - 20190317 by   Massimiliano Scarano   mscarano@libero.it\n";



const UBYTE MsgText[] =
"VOID PROUDLY PRESENTS MENACE BY PSYGNOSIS 1988, LMB TO EXIT.\
 CRACK INTRO V1.0 20190317 RELEASED FOR REVISION 2019.\
 CREDITS:\
...
*/
  ...
  for ( K = 0; K < 2; K++ )
  {
    if ( ( DBuf[ K ] = ( UBYTE* ) AllocMem( ( BITPLANE320x256 * 3 ) + SCRATCHAREA + ( PADDINGAREA * 2 ), MEMF_CHIP | MEMF_CLEAR ) ) == NULL )
    {
      fprintf( stderr, "%s\n", "AllocMem() failed" );
      exit( EXIT_FAILURE );
    } /* if */
  ...
  for ( K = 0; K < 2; K++ )
  {
    if ( DBuf[ K ] )
    {
      FreeMem( DBuf[ K ], ( BITPLANE320x256 * 3 ) + SCRATCHAREA + ( PADDINGAREA * 2 ) );
    } /* if */
  } /* for */
...
function void ScrollText( UBYTE OffScreenBuffer )
{
  ...
  Scroll( OffScreenBuffer );   SineScroll( OffScreenBuffer );
...
function void PrintChar( UBYTE OffScreenBuffer, UBYTE CurrChar )
{
  ...
  Hardware_p->bltdpt = ( APTR ) &( DBuf[ OffScreenBuffer ][ 30760 + PADDINGAREA ] ); /* address of destination (fixed top left corner in the bitplane) */
...
function void Scroll( UBYTE OffScreenBuffer )
{
  ...
  switch ( OffScreenBuffer )
  {
    ...
    case 0:
      ...
      Hardware_p->bltapt = ( APTR ) &( DBuf[ 0 ][ 31390 + PADDINGAREA ] ); /* address of source (bitplane) */
      Hardware_p->bltdpt = ( APTR ) &( DBuf[ 1 ][ 31390 + PADDINGAREA ] ); /* address of destination (bitplane) */
      break;

    case 1:
      /* source and destination are equal */
      Hardware_p->bltapt = ( APTR ) &( DBuf[ 1 ][ 31390 + PADDINGAREA ] ); /* address of source (bitplane) */
      Hardware_p->bltdpt = ( APTR ) &( DBuf[ 0 ][ 31390 + PADDINGAREA ] ); /* address of destination (bitplane) */
      break;
  } /* switch */
...
function void PrintCredits( void )
{
  ...
  fprintf( stdout, "%s\n", "Font: rinkydinkelectronics.com" );
  fprintf( stdout, "%s\n", "Ship: dmadesign.org" );
  fprintf( stdout, "%s\n", "Music: Accel   amp.dascene.net" );
  fprintf( stdout, "%s\n", "P6112 music player routine: Photon and others" );

  fprintf( stdout, "\n%s\n", "Greetings:" );
  fprintf( stdout, "%s\n", "Pchelka777, Blue Unicorn, Vinsoft, Invent, Fractal Minds, Doctor Fake, Doctor Mabuse, Hellsoft," );
  fprintf( stdout, "%s\n", "all members of VOID and our friends of Nukleus." );
...
/*
  - 1 pixel sine scroller.
    ...
    * making every 16th blit a copy-blit instead of an OR-blit (done)
...
{
  const UWORD SliceMask[] = { 0x8000, 0x4000, 0x2000, 0x1000, 0x0800, 0x0400, 0x0200, 0x0100,
  ...
  for ( Source = 2; Source < 18; Source++ ) /* for ( Source = 0; Source < 20; Source++ ) */
  {
    ...
    /* first pixel slice of 16:copy operation */
    /* copy from channel A to channel D, no shifts, ascending mode */
    Hardware_p->bltcon0 = 0x9f0;

    Hardware_p->bltalwm = SliceMask[ 0 ];
    ...
    switch ( OffScreenBuffer )
    {
      ...
      case 0:
        ...
        Hardware_p->bltdpt = ( APTR ) &( DBuf0_p[ SineValue - PADDINGBPLAREA + Source + Source ] ); /* address of destination (top left corner) */
        break;

      case 1:
        Hardware_p->bltapt = ( APTR ) &( DBuf0_p[ 30720 + Source + Source ] ); /* address of source (top left corner) */
        Hardware_p->bltdpt = ( APTR ) &( DBuf1_p[ SineValue - PADDINGBPLAREA + Source + Source ] ); /* address of destination (top left corner) */
        break;
    } /* switch */

    Hardware_p->bltsize = ( ( PADDINGH + 16 + PADDINGH ) << 6 ) + 1; /* UWORD size */
    ...
    for ( Slice = 1; Slice < 16; Slice++ )
    {
      ...
      Hardware_p->bltalwm = SliceMask[ Slice ];

      switch ( OffScreenBuffer )
      {
        case 0:
          Hardware_p->bltapt = ( APTR ) &( DBuf1_p[ 30720 + PADDINGAREA + Source + Source ] ); /* address of source (top left corner) */
        ...
        case 1:
          Hardware_p->bltapt = ( APTR ) &( DBuf0_p[ 30720 + PADDINGAREA + Source + Source ] ); /* address of source (top left corner) */
