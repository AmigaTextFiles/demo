/*
  Menace_cracktro.c V1.0 20190324
...
const BYTE ID[] = "$VER: Menace_cracktro V1.0 - 20190324 by   Massimiliano Scarano   mscarano@libero.it\n";



const UBYTE MsgText[] =
"VOID PROUDLY PRESENTS MENACE BY PSYGNOSIS 1988, LMB TO EXIT.\
 CRACK INTRO V1.0 20190324 RELEASED FOR REVISION 2019.\
 CREDITS:\
...
function void Scroll( UBYTE OffScreenBuffer )
{
  ...
  Hardware_p->bltapt = ( APTR ) &( DBuf[ OffScreenBuffer ][ 31390 + PADDINGAREA ] ); /* address of source (bitplane) */
  Hardware_p->bltdpt = ( APTR ) &( DBuf[ OffScreenBuffer ^ 1 ][ 31390 + PADDINGAREA ] ); /* address of destination (bitplane) */
  ...
  Hardware_p->bltsize = 1045; /* ( 16 << 6 ) + 21; */ /* UWORD size */
...
function void SineScroll( UBYTE OffScreenBuffer )
{
  ...
  register UBYTE Source, Slice, SinePos;
  ...
  register UWORD ExtraBlitSize = ( ( PADDINGH + 16 + PADDINGH ) << 6 ) + 1; /* UWORD size */
