...
const BYTE ID[] = "$VER: HelloVOID V1.0 - 20180225 by   Massimiliano Scarano   mscarano@libero.it\n";



const UBYTE MsgText[] =
"ALIENTECH OF VOID PROUDLY PRESENTS A DEMO CALLED HELLOVOID, LEFT MOUSE BUTTON TO EXIT.\
 DEMO V1.0 20180225 RELEASED FOR REVISION 2018.\
...
function int main( int argc, char* argv[] )
{
  ...
  if ( ( argc == 2 ) && ( strcmp( argv[ 1 ], "turbo" ) == 0 ) )
  {
    IsTurbo = TRUE;
  } /* if */
  else
...
*/
  ...
  /* load module */
  if ( ( Mod_p = ReadFile( "DemoData/P61.Dolphins-Dreamquest-by-Esau", &ModSize, MEMF_CHIP | MEMF_CLEAR ) ) == NULL )
  {
...
function void SineScroll( UBYTE OffScreenBuffer )
{
  ...
  {
    if ( IsTurbo == TRUE ) { Mask = 0x8000; } else { Mask = 0xf000; };

    /* for all the slices of N pixel each */
    for ( Slice = 0; ( IsTurbo == TRUE ) ? Slice < 16 : Slice < 4; Slice++ )
    {
      /* generate a column */
      ...
      if ( IsTurbo == TRUE ) { Mask >>= 1; } else { Mask >>= 4; }
    } /* for */
  } /* for */
