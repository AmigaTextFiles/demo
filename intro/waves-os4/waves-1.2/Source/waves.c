...
void step_height( buffer *v, buffer *z0, buffer *z1 ) {
	...
	for( y=1 ; y<y1 ; y++, z1_data+=2, z0_data+=2, v_data+=2 )
		for( x=1 ; x<x1 ; x++, z1_data++, z0_data++, v_data++ )
			*z1_data = *z0_data + *v_data; // simple, aint it?
...
void lissajous( buffer *b ) {
	...
	a = pow( a, 7 );
	x = (0.5 + 0.48*sin(5.0*lissajous_angle) + 0.02*a*cos(100.0*lissajous_angle)*sin(7.0*lissajous_angle) )*b->x;
	y = (0.5 + 0.48*cos(7.0*lissajous_angle) + 0.02*a*cos(100.0*lissajous_angle)*cos(5.0*lissajous_angle) )*b->y;
...
void blossom( buffer *b ) {
	...
	for( a=0.0 ; a<2.0*M_PI ; a+=0.001 )
		deform(
			b->x * (0.5 + (blossom_r*sin(blossom_i*blossom_m*a+blossom_a)+(1.0-blossom_r)*sin(blossom_k*blossom_n*a+blossom_a))/2.2 ),
			b->y * (0.5 + (blossom_r*cos(blossom_j*blossom_m*a+blossom_a)+(1.0-blossom_r)*cos(blossom_l*blossom_n*a+blossom_a))/2.2 ),
			(1<<5),
			b
		);
}
...
void rain( buffer *b ) {
	...
	deform(
		(b->x-1.0)*rand()/RAND_MAX,
		(b->y-1.0)*rand()/RAND_MAX,
		((float)(1<<12)) * rand() / RAND_MAX,
		b
	);
}
...
int main( int argc, char *argv[] ) {
	...
	while( !quit ) {
		...
		/* who cares? */
		printf("\rsmooth=%2.3f   gravity=%2.3f   friction=%2.3f   elastics=%2.3f",
			smooth,gravity,friction,elastics);
	}
	printf("\n");
