...
void main(void)
{
	...
	if (scalebuffer=(UBYTE *) AllocMem(CHUNKYWIDTH*CHUNKYHEIGHT*2,MEMF_PUBLIC|MEMF_CLEAR)) {

	fb=framebuffer+CHUNKYWIDTH, fb2=framebuffer2+CHUNKYWIDTH;
	...
	c2p2x1_8_c5_030_init(CHUNKYWIDTH,CHUNKYHEIGHT*2,0,0,CHUNKYWIDTH/4,(LONG)renderbm.BytesPerRow);
	...
	while (!MouseButton()) 
	{
		...
		{
			fire(framebuffer3);
	 		bump(fb2,framebuffer3+CHUNKYWIDTH);
			display(scalebuffer,fb2);
			Z=1.0; K=1;
		}
		else if (t<140)
	...
	FreeMem(scalebuffer,CHUNKYWIDTH*CHUNKYHEIGHT*2);
	}
...
{
	scaley((ULONG *)p1,(ULONG *)p2);
	c2p2x1_8_c5_030(p1,renderbm.Planes[0]);
}
