...
SDL_WM_SetCaption("Screen Art by Nathan Stenzel , OS4 Port by ToAks", "ScreenArt");
        for(i=0;i<64;i++){
                colorWheel[i].r=i;
                colorWheel[i].g=0;
                colorWheel[i].b=0;
    ...
    if ( SDL_MUSTLOCK(screen) ) {
        ...
        for (i=0; i<256; i++) colors[i]=colorWheel[i];
        SDL_SetColors(screen, colors, 0, 256);

        for (y=0; y<=256;y++) for (x=0; x<=y; x++) {
                c=((x+y+x*y)/2)&0xff;
...
while ( SDL_PollEvent(NULL) == 0 ) {
	...
	if ( SDL_LockSurface(screen) < 0 ) {
        ...
        for (i=0; i<256;i++) colors[i]=colorWheel[(i+c)&0xff];
        c+=2;
        SDL_SetColors(screen, colors, 0, 256);
 }
}
