...
VLB	equ	1		;This Version needs the System to be killed. And you have to
		;jump to "tp_play" every Vertical Blank !
CIA	equ	0		;This Version also needs the System to be killed. But will
		;replay the module on its own, as long as the level 6 Interrupt
		;is active.
SystemCIA	equ	-1	;Yeah, and this one replays the Module with all the wonderful
		;advantages of real CIA-timing. And it uses real System-
		;interrupts for it. So it will run perfect with Multitasking !

replaymode	equ	CIA	;what kind of replaying do you want to use ?

pt1.1	equ	no	;protracker v1.1 compatible (default=pt2.0)
syncs	equ	no	;do you use vibrato or tremolo with sync ?
funk	equ	no		;do you use the ef-comand ?
vbruse	equ	yes	;use vectortableoffset ?
volume	equ	no	;use volumesliding ?
split	equ	yes	;use a splitted Song- and Samplefiles
choosestart	equ	no	;do you want to start the song from any other point than the
		;beginning ?
switch	equ	no	;do you want to switch ON/OFF any voices ?
suck	equ	yes	;das ya dick wanna b suckd ?
...
tp_noupperinst:
	...
	move.w	d1,d5
	lsr.w	#4,d5
	lea	_tp_triggered,a4
	and.w	#$1f,d5
	move.b	#1,0(a4,d5.w)
	movem.l	84(a0,d1.w),d5-d7/a4	; tp_instlist-tp_wait-16
...
tp_voicefx4sine:
	lsr	#2,d0
	move.b	68(a0,d0.w),d0	; tp_vibratolist-tp_wait
tp_voicefx4set:
	mulu	30(a1),d0
	lsr	#7,d0
	tst.b	29(a1)
...
tp_int1pon:	dc.l		0
tp_int3pon:	dc.l		0
tp_newpattpos:	dc.w	0
tp_pattdelay:	dc.w		0
tp_pattrepeat:	dc.w	0
tp_pattadrpon:	dc.w	0
	ifne	choosestart
tp_patternnumber:	dc.w	0
	endc
_tp_data:
tp_data:	dc.l		0
	ifne	split
...
tp_vbr:	dc.l		0
	endc
	ifne	volume
tp_volume:	dc.w		255		;0=off,255=max. volume
	endc
	iflt	replaymode
