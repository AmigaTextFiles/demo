;*---------------------------------------------------------------------------
;  :Program.    Bamiga Sector One & Cybertech
;  :Contents.   "Grand Prix Cirquit" Crack-Intro (198x)
;  :Author.     Stefano "Max Headroom/TEX" Pucino
;  :Requires.   Bamiga Sector One Intro (ByteKiller v1.3, 48.840 bytes)
...
;                NEW: Removed access- fault (BYTE-Write: $BFD100)
;---------------------------------------------------------------------------*
...
;======================================================================
    ...
    IFD BARFLY
        OUTPUT  "BamigaSectorOne-GPC.slave" ; Name of the slave
...
_name                                       ; Full name of the intro
    dc.b    "Grand Prix Circuit Crack-Intro",0
_copy                                       ; Copyright information
    dc.b    "198x Bamiga Sector One & Cybertech",0
_info                                       ; Who am I ? ;)
    dc.b    "-----------------------------",10
...
; to directly load and relocate an executable for out needs.
...
BUGADR     = $336e8                         ; Address of the SoundTracker-loop
INTROSTART = $30000                         ; Start-address of the intro
...
; Remove access fault of BYTE-Write to $BFD100 which is not needed.

    move.w  #$4e71,INTROSTART+$279e         ; NOP: "LEA ($bfd100),a1"
    move.w  #$4e71,INTROSTART+$27a0
    move.w  #$4e71,INTROSTART+$27a2

    move.w  #$4e71,INTROSTART+$27a4         ; NOP: "ori.b #$f8,(a1)"
    move.w  #$4e71,INTROSTART+$27a6

    move.w  #$4e71,INTROSTART+$27ae         ; NOP: "andi.b #$87,(a1)"
    move.w  #$4e71,INTROSTART+$27b0
    jmp     INTROSTART                      ; Now start the intro
...
; But how many "loops" should I use fo this routine ???
; To say the truth... I don't know eighter ^_^
; You can be sure that one loop is not enough. Most of the time it's
; between 3 and 5 iterations. Try it with 3 first. If you encounter
; problems (blitter flickers, sample skips) increase the number by one and
; try again.
; This highly sophisticated method of programming is called "Try And Error"
; and should not be used by a professional software house ^_^.
;======================================================================
...
_exe                                        ; Name of the Intro
    dc.b    "bs1-crackgrandprixcircuit",0
    even
