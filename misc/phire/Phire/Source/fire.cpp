...
void fire::draw() {
  ...
  for (tx = 0; tx < rad; tx++) {
    for (ty = 0; ty < rad; ty++) {
      ...
      if (ty) putp(int(x + tx), int(y - ty), int(time * tmp * 96));
      if (tx) putp(int(x - tx), int(y + ty), int(time * tmp * 96));
      if (tx && ty) putp(int(x - tx), int(y - ty), int(time * tmp * 96));
    }
  }
}
